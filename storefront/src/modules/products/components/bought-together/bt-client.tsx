...
export default function BoughtTogetherClient({
  ...
  const { extras, toggleExtra, setBoughtTogetherPrice, setBoughtTogetherName } =
    useCombinedCart()
  ...
  const handleToggleExtra = useCallback(
    (variant: any, product: any) => {
      const variantId = variant.id

      if (extras.includes(variantId)) {
        // Si ya está seleccionado, removerlo
        toggleExtra(variantId)
      } else {
        // Si no está seleccionado, agregarlo
        toggleExtra(variantId)
        setBoughtTogetherName(variantId, product.title)
      }
    },
    [extras, toggleExtra, setBoughtTogetherName]
  )
      ...
      {products.map((p) => {
        ...
        return (
          <div
            ...
            onClick={() => handleToggleExtra(variant, p)}
          >
            {/* Layout desktop */}
