...
import { ChevronLeft, AlertTriangle, Trash, Send, X, CheckCircle, SearchIcon } from "lucide-react";
...
        {/* Sección derecha: Controles, Badges y Visualizaciones */}
...
          </select>
          {/* Los componentes OrderVisualization y FinancingVisualization se mantienen dentro del flex-wrap */}
          <OrderVisualization chatData={data} userId={userid} />
          <FinancingVisualization userId={userid!} />
        </div>
      </div>
...
        {userMessages
          .sort(
            (a, b) => new Date(a.created_at).getTime() - new Date(a.created_at).getTime()
          )
          .map((msg, index) => (
            <ChatBubble 
              key={msg.id} 
