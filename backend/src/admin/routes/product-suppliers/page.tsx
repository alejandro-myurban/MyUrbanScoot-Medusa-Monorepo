...
const ProductSuppliersPage = () => {
  ...
  // 🔧 HOOKS MOVIDOS AQUÍ - Reset página cuando cambian los filtros
  useEffect(() => {
    setCurrentPage(1);
  }, [filterBySupplier, filterByPreferred, filterByWorkshop, searchTerm]);

  // 🔧 HOOKS MOVIDOS AQUÍ - Si se solicitó mostrar historial pero no se encuentra la relación, resetear el estado
  useEffect(() => {
  ...
  }, [showPriceHistory, relationsData]);

  // 🔧 LÓGICA DE DATOS DESPUÉS DE TODOS LOS HOOKS
  const relations: ProductSupplier[] = relationsData?.product_suppliers || [];
  ...
  // Buscar la relación para el historial
  const selectedRelationForHistory = showPriceHistory ? relations.find(r => r.id === showPriceHistory) : null;

  // 🔧 RENDER CONDICIONAL CON JSX - Vista de historial de precios
  if (showPriceHistory && selectedRelationForHistory) {
    return (
      <Container className="divide-y p-0">
        <div className="flex items-center justify-between px-6 py-4">
