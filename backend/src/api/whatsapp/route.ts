...
    // Guarda el mensaje del usuario con el estado actual (IA o AGENTE)
    await chatService.saveMessage({
        user_id: userId,
        message: incomingMsg,
        role: "user",
...
    // Si el usuario pide asistencia personal
...
      // Guarda el mensaje de confirmación del asistente en el historial
      await chatService.saveMessage({
        user_id: userId,
...
    }
    
    // Si el estado es "AGENTE", no procesamos con la IA.
    if (lastStatus === "AGENTE") {
        console.log(`💬 Mensaje recibido de ${userId} en modo AGENTE. No se procesa con IA.`);
        // El mensaje del usuario ya se ha guardado al principio
        return res.status(200).send("<Response></Response>");
    }
